...
dados_obitos_maternos <- read_delim("dados_oobr_obitos_grav_puerp_maternos_oficiais_1996_2025.csv", 
                                    delim = ",", escape_double = FALSE, trim_ws = TRUE) |>
  clean_names()

dados_obitos_maternos_estendido <- read_delim("dados_oobr_obitos_grav_puerp_analise_cruzada_1996_2025.csv", 
                                    delim = ",", escape_double = FALSE, trim_ws = TRUE) |>
  clean_names()

dados_obitos_desconsiderados <- read_delim("dados_oobr_obitos_grav_puerp_desconsiderados_1996_2025.csv", 
                                           delim = ",", escape_double = FALSE, trim_ws = TRUE) |>
  clean_names()

dados_obitos_por_uf <- read_delim("dados_oobr_obitos_grav_puerp_ufs_1996_2025.csv", 
                                           delim = ",", escape_double = FALSE, trim_ws = TRUE) |>
  clean_names()

dados_garbage_codes <- read_delim("dados_oobr_obitos_grav_puerp_garbage_codes_1996_2025.csv", 
                                           delim = ",", escape_double = FALSE, trim_ws = TRUE) |>
  clean_names()
...
É um painel de visualização dinâmico com análises acerca da mortalidade de gestantes e puérperas no Brasil, sob a perspectiva de algumas variáveis da base de dados do Sistema de Informações sobre Mortalidade (SIM). As informações consideradas são referentes aos registros de óbitos de mulheres gestantes ou puérperas dos anos de 1996 a 2025 **(sendo estes preliminares)**. 
...
São providos do DATASUS do Ministério da Saúde e formam um grande banco anual composto por todos os registros das declarações de óbitos (o SIM) que está disponível a partir do ano de 1996. Atualmente, os dados do SIM estão disponíveis até 2025 **(sendo estes preliminares)**. Os dados deste painel são atualizados na última sexta feira de cada mês, tendo a última atualização ocorrido em `r data_por_extenso`. Os dados podem ser acessados em: Herzog, R. S.; Francisco, R. P. V.; Rodrigues, A. S.  Óbitos de gestantes e puérperas [banco de dados], 2022, Observatório Obstétrico Brasileiro (OOBr). Disponível em DOI: https://doi.org/10.7303/syn42902915.
...
p(em("Obs.: os dados de 2025 são preliminares."))
...
numericInput(
  ...
  value = max(dados_obitos_maternos$ano),
...
Óbitos maternos oficiais preenchidos com garbage codes {data-navmenu="Visualizações"}
================================================================================

Inputs {.sidebar data-width=350 style="width: 350px; visibility: visible; top: 75px;"}
------------------------------------- 
```{r input-obitosGC}
hr();h5(span("Óbitos maternos oficiais preenchidos com garbage codes", style = "font-weight: bold;")); hr()
p(em("Obs.: os dados de 2025 são preliminares."))
...
numericInput(
  inputId = "selectAnoGC",
  label = HTML("<span class = 'negrito'> Selecione o ano de análise: </span>"),
  value = max(dados_garbage_codes$ano),
  min = min(dados_garbage_codes$ano),
  max = max(dados_garbage_codes$ano)
)

selectInput(
  inputId = "selectNivelGC",
  label = HTML("<span class = 'negrito'> Selecione o nível de análise: </span>"),
  choices = c("Nacional", "Estadual", "Municipal"),
  selected = "Nacional"
)
...
conditionalPanel(
  condition = "input.selectNivelGC == 'Nacional'",
  checkboxGroupInput(
    inputId = "selectRegiaoGC",
    label = HTML("<span class = 'negrito'> Selecione as regiões de análise: </span>"),
    choices = c("Norte", "Nordeste", "Centro-Oeste", "Sudeste", "Sul"),
    selected = c("Norte", "Nordeste", "Centro-Oeste", "Sudeste", "Sul")
  )
...
conditionalPanel(
  condition = "input.selectNivelGC == 'Estadual'",
  selectInput(
    inputId = "selectEstadoGC",
    label = HTML("<span class = 'negrito'> Selecione o estado: </span>"),
    choices = estadosChoices,
    multiple = FALSE
    )
...
conditionalPanel(
    condition = "input.selectNivelGC == 'Municipal'",
    selectizeInput(
      inputId = "selectEstadoMuniGC",
      label = HTML("<span class = 'negrito'> Selecione o estado ao qual pertence o município: </span>"),
      choices = estadosChoices,
      options = list(placeholder = "Selecione um estado")
    ),
    selectizeInput(
      inputId = "selectMunicipioGC",
      label = HTML("<span class = 'negrito'> Selecione o município: </span>"),
      choices = NULL,
      options = list(placeholder = "Selecione um município")
    )
...
observeEvent(input$selectEstadoMuniGC, {
  updateSelectizeInput(
    inputId = "selectMunicipioGC",
    choices = sort(tabela_aux_municipios |> dplyr::filter(uf == input$selectEstadoMuniGC) |> pull(municipio) |> unique())
  )
})
...
checkboxGroupInput(
  inputId = "selectRacaGC",
  label = HTML("<span class = 'negrito'> Selecione a raça/cor da gestante ou puérpera: </span>"),
  choices = sort(unique(dados_garbage_codes$racacor)),
  selected = sort(unique(dados_garbage_codes$racacor))
)

checkboxGroupInput(
  inputId = "selectCausasGC",
  label = HTML("<span class = 'negrito'> Selecione os tipos de causas obstétricas: </span>"),
  choices = sort(unique(dados_garbage_codes$tipo_de_morte_materna)),
  selected = sort(unique(dados_garbage_codes$tipo_de_morte_materna))
)

checkboxGroupInput(
  inputId = "selectPeriodoGC",
  label = HTML("<span class = 'negrito'> Selecione os períodos de óbito: </span>"),
  choices = sort(unique(dados_garbage_codes$periodo_do_obito)),
  selected = sort(unique(dados_garbage_codes$periodo_do_obito))
)

checkboxGroupInput(
  inputId = "selectInvestigacaoGC",
  label = HTML("<span class = 'negrito'> Selecione o tipo de investigação do óbito: </span>"),
  choices = c("Investigado por Comitê de Morte Materna" = "Sim", 
              "Não investigado por Comitê de Morte Materna" = "Não",
              "Sem informação"),
...
conditionalPanel(
  condition = "input.selectDownloadGC == 'Sim'",
  selectInput(
    inputId = "selectArquivoGC",
    label = HTML("<span class = 'negrito'> Selecione o tipo do arquivo: </span>"),
    choices = c("CSV", "XLSX"),
    selected = "CSV"
  ),
  conditionalPanel(
    condition = "input.selectArquivoGC == 'CSV'",
    downloadHandler(
      filename = reactive({
        case_when(
          input$selectNivelGC == "Nacional" ~ paste0("obitos_maternos_", janitor::make_clean_names(input$selectNivelGC), input$selectAnoGC, ".csv"),
          input$selectNivelGC == "Estadual" ~ paste0("obitos_maternos_", janitor::make_clean_names(input$selectEstadoGC), input$selectAnoGC, ".csv"),
          input$selectNivelGC == "Municipal" ~
            paste0("obitos_maternos_", janitor::make_clean_names(gsub(" ", "_", gsub(" - ", " ", input$selectMunicipioGC))), input$selectAnoGC, ".csv")
        )
        }),
      content = function(file) {
        write.table(dados_gc_filtrados(), file, sep = ";", row.names = FALSE, fileEncoding = "UTF-8")
      }
    )
  ),
  conditionalPanel(
    condition = "input.selectArquivoGC == 'XLSX'",
    downloadHandler(
      filename = reactive({
        case_when(
          input$selectNivelGC == "Nacional" ~ paste0("obitos_maternos_", janitor::make_clean_names(input$selectNivelGC), input$selectAnoGC, ".xlsx"),
          input$selectNivelGC == "Estadual" ~ paste0("obitos_maternos_", janitor::make_clean_names(input$selectEstadoGC), input$selectAnoGC, ".xlsx"),
          input$selectNivelGC == "Municipal" ~
            paste0("obitos_maternos_", janitor::make_clean_names(gsub(" ", "_", gsub(" - ", " ", input$selectMunicipioGC))), input$selectAnoGC, ".xlsx")
        )
        }),
      content = function(file) {
        write.xlsx(dados_gc_filtrados(), file, sep = ";", row.names = FALSE, fileEncoding = "UTF-8")
      }
    )
  )
)

dados_gc_filtrados <- reactive({
  if (input$selectNivelGC == "Municipal") {
    req(input$selectEstadoMuniGC)
  }
  if (input$selectNivelGC == "Nacional") {
    if (length(input$selectRegiaoGC) == 5) {
      dados_garbage_codes |>
        filter(ano == input$selectAnoGC,
               tipo_de_morte_materna %in% input$selectCausasGC,
               periodo_do_obito %in% input$selectPeriodoGC,
               investigacao_cmm %in% input$selectInvestigacaoGC,
               racacor %in% input$selectRacaGC,
               idade >= input$selectIdadeGC[1] & idade <= input$selectIdadeGC[2]) |>
        mutate(regiao = "Todas") |>
        group_by(ano, regiao, capitulo_cid10, causabas_categoria, tipo_de_morte_materna, periodo_do_obito, racacor, investigacao_cmm) |>
        summarise(obitos = sum(as.numeric(obitos))) |>
        ungroup()
      ...
      dados_garbage_codes |>
        filter(ano == input$selectAnoGC,
               regiao %in% input$selectRegiaoGC,
               tipo_de_morte_materna %in% input$selectCausasGC,
               periodo_do_obito %in% input$selectPeriodoGC,
               investigacao_cmm %in% input$selectInvestigacaoGC,
               racacor %in% input$selectRacaGC,
  ...
  } else if (input$selectNivelGC == "Estadual") {
    dados_garbage_codes |>
      filter(ano == input$selectAnoGC,
             uf == unique(tabela_aux_municipios$sigla_uf[which(tabela_aux_municipios$uf == input$selectEstadoGC)]),
             tipo_de_morte_materna %in% input$selectCausasGC,
             periodo_do_obito %in% input$selectPeriodoGC,
             investigacao_cmm %in% input$selectInvestigacaoGC,
             racacor %in% input$selectRacaGC,
      ...
      summarise(obitos = sum(as.numeric(obitos))) |>
      ungroup() 
  } else if (input$selectNivelGC == "Municipal") {
    dados_garbage_codes |>
      filter(ano == input$selectAnoGC,
             municipio == input$selectMunicipioGC,
             uf == unique(tabela_aux_municipios$sigla_uf[which(tabela_aux_municipios$uf == input$selectEstadoMuniGC)]),
             tipo_de_morte_materna %in% input$selectCausasGC,
             periodo_do_obito %in% input$selectPeriodoGC,
             investigacao_cmm %in% input$selectInvestigacaoGC,
             racacor %in% input$selectRacaGC,
...
### <span style = 'font-weight: 700; color: black'>Tabela de óbitos maternos oficiais preenchidos com garbage codes</span>

```{r tabela_gc}
dados_tabela_gc <- reactive({
  if (input$selectNivelGC == "Nacional") {
    dados_gc_filtrados() |>
      select(!c(ano, regiao))
  } else if (input$selectNivelGC == "Estadual") {
...
renderReactable({
  validate(
    need(nrow(dados_tabela_gc()) != 0, message = "Não existem registros de óbitos maternos preenchidos com garbage codes para os filtros selecionados.")
  )
  if (nrow(dados_tabela_gc()) > 0) {
    dados_tabela_gc() |>
      reactable(
        groupBy = c("capitulo_cid10", "causabas_categoria"),
        columns = list(
...
numericInput(
  ...
  label = HTML("<span class = 'negrito'> Selecione o ano de análise: </span>"),
  value = max(dados_obitos_desconsiderados$ano),
...
numericInput(
  inputId = "selectAnoUF",
  label = HTML("<span class = 'negrito'> Selecione o ano de análise: </span>"),
  value = max(dados_obitos_por_uf$ano),
...
sliderInput(
  ...
  value = c("2017", max(dados_obitos_maternos_estendido$ano)),
  min = min(dados_obitos_maternos_estendido$ano),
  max = max(dados_obitos_maternos_estendido$ano),
  sep = "",
)
...
variaveis <- c(
  ...
  "Necrópsia" = "necropsia",
  "Capítulo da CID-10" = "capitulo_cid10",
...
checkboxGroupInput(
  ...
  label = HTML("<span class = 'negrito'> Selecione a raça/cor da gestante ou puérpera: </span>"),
  choices = sort(unique(dados_obitos_maternos_estendido$racacor)),
  selected = sort(unique(dados_obitos_maternos_estendido$racacor))
)
